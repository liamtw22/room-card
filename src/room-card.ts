...
export class RoomCard extends LitElement {
  ...
  // Public getter for config to allow HA infrastructure to access it
  public get config(): RoomCardConfig {
    return this._config;
  }
  ...
  protected shouldUpdate(changedProperties: PropertyValues): boolean {
    ...
    if (changedProperties.has('hass')) {
      ...
      // If we don't have old hass, we should update
      if (!oldHass) {
        return true;
      }

      // Check if any of our tracked entities have changed
      ...
      this.devices.forEach(device => {
        if (device.entity) {
          entitiesToCheck.push(device.entity);
        }
        if (device.control_entity) {
          entitiesToCheck.push(device.control_entity);
        }
      });

      // Add entities used for dynamic colors
